...
export default function FancyCard({
  ...
  return (
    <div className={`relative w-full rounded-xl bg-gradient-to-br from-white to-gray-50 text-gray-700 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 ${isSide ? "md:flex md:flex-row" : "flex flex-col"}`}>
      {/* Media */}
      <div
        className={`relative overflow-hidden rounded-xl bg-clip-border shadow-lg group ${isSide ? `w-full md:w-1/3 h-48 md:h-auto ${mediaOrder} mx-4 my-4` : "h-40 mx-4 -mt-6"}`}
      ...
      <div className={`p-6 ${isSide ? `md:w-2/3 ${contentOrder}` : ""}`}>
        ...
        {buttonText && (
          <div className="pt-6">
            <a href={buttonHref} className="group inline-flex items-center gap-2 px-6 py-3 font-bold text-white rounded-lg bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 shadow-lg shadow-blue-500/30 hover:shadow-blue-500/40 transition-all duration-300 hover:-translate-y-0.5">
